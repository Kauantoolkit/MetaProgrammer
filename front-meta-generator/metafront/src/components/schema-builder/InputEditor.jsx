...
export default function InputEditor({ functionality, onUpdate }) {
  const [newFieldName, setNewFieldName] = useState('');
  const [newFieldType, setNewFieldType] = useState('String');

  const inputFields = functionality.input || [];
  ...
  const removeField = (index) => {
    const updatedInput = inputFields.filter((_, i) => i !== index);
    const updatedFunctionality = {
      ...functionality,
      input: updatedInput
    };
  ...
  const updateField = (index, field) => {
    const updatedInput = inputFields.map((f, i) => i === index ? field : f);
    const updatedFunctionality = {
      ...functionality,
      input: updatedInput
    };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-slate-200">Campos de Entrada</h3>
        <span className="text-sm text-slate-500">{inputFields.length} campos</span>
      </div>
      ...
      <div className="space-y-3">
        ...
        {inputFields.length === 0 && (
          <div className="text-center py-8 text-slate-500">
            ...
            <p>Nenhum campo de entrada definido</p>
