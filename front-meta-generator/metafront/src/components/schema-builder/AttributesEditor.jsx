...
export default function AttributesEditor({ entity, onUpdate }) {
  ...
  const handleAddAttribute = () => {
    ...
    const newAttr = {
      name: `campo${currentAttrs.length + 1}`,
      type: 'string',
      constraints: []
    };
  ...
  const handleDeleteAttribute = (index) => {
    ...
    onUpdate({
      ...entity,
      attributes: currentAttrs.filter((_, i) => i !== index)
    });
  };
  ...
  const handleToggleConstraint = (index, constraint) => {
    ...
    const currentConstraints = attr.constraints || [];

    const hasConstraint = currentConstraints.some(c =>
      typeof c === 'string' ? c === constraint : c.startsWith(constraint)
    );

    let newConstraints;
    if (hasConstraint) {
      newConstraints = currentConstraints.filter(c =>
        typeof c === 'string' ? c !== constraint : !c.startsWith(constraint)
      );
    } else {
      newConstraints = [...currentConstraints, constraint];
    }
  ...
  return (
    <div className="space-y-3">
      {(entity.attributes || []).map((attr, index) => {
        const safeConstraints = attr.constraints || [];
              ...
              <div className="flex-1 min-w-0">
                ...
                {safeConstraints.length > 0 && (
                  <div className="flex gap-1 mt-1 flex-wrap">
                    {safeConstraints.slice(0, 3).map((c, i) => (
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {CONSTRAINTS.map((constraint) => {
                      const isActive = safeConstraints.includes(constraint.value);
                      return (
                        <button
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      type="number"
                      placeholder="min"
                      value={safeConstraints.find(c => c.startsWith?.('min:'))?.split(':')[1] || ''}
                      onChange={(e) => {
                        const newConstraints = safeConstraints.filter(c => !c.startsWith?.('min:'));
                  ...
                  <div>
                    ...
                    <Input
                      type="number"
                      placeholder="max"
                      value={safeConstraints.find(c => c.startsWith?.('max_length:') || c.startsWith?.('max:'))?.split(':')[1] || ''}
                      onChange={(e) => {
                        const newConstraints = safeConstraints.filter(c => !c.startsWith?.('max_length:') && !c.startsWith?.('max:'));
